...
| **属性矩阵** | **24 项细分属性**（11 核心 + 13 扩展），覆盖专业力、逻辑、表达、抗压、金钱、导师好感度、数据分析、商业嗅觉等 |
...
| **日常情境寒暄** | 每次拜访 NPC 均自动随机触发专属 3~4 轮日常对话，并支持随时点击 `💬 寒暄聊两句` 深度畅聊 |
| **微信全双工同步** | 校园地图线下拜访与电脑端「微信社交系统」的好感度、解锁状态与聊天记录全双工实时同步 |
| **分支事件** | **77 条深度分支事件**（覆盖 `e01` ~ `e86` 号大类剧情），每条含 3-4 个预设选择，选择前隐藏影响，结算后展示详细数值变动 |
...
| **存档与助手** | 本地自动保存 + **3 个手动存档槽**；内置「建哥 AI 军师」，随时提供游戏攻略与现实转行答疑 |
...
(5位NPC+导师AVG)      (17项成长行动)         (77条深度抉择 / AI自由行动)
    └──────────────────────┬──────────────────────┘
                           ↓
                    数值结算 + 月度账单
                           ↓
...
进入游戏后，点击右下角的 **「建哥 AI 军师」** 齿轮图标，即可在设置面板中填入你的 **ModelScope (魔搭) API Key**。
